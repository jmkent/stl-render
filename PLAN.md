# STL Renderer Project Plan

## Goal

Build a standalone Rust binary that renders STL files to deterministic 2D PNG previews.

```bash
stl-render input.stl -o preview.png --view iso --width 512 --height 512
```

## Scope

- Input: STL (binary and ASCII), up to 500MB+
- Output: PNG
- Rendering: software rasterizer, headless
- Packaging: static binary, no runtime deps

## Non-Goals

- Slicer, mesh repair, CAD viewer
- GPU rendering
- Multi-format support (OBJ, 3MF, STEP, GLTF)
- Python bindings (future phase)

## Design Decisions

| Decision | Choice | Rationale |
|----------|--------|-----------|
| Rendering backend | Software rasterizer | Headless, deterministic, no GPU deps |
| STL parsing | Custom streaming parser | Zero deps, handles 500MB+ via mmap |
| Determinism | Same-machine | Cross-platform not required |
| Math library | glam | Fast, ergonomic, minimal |

## CLI Specification

```bash
stl-render <INPUT> -o <OUTPUT> [OPTIONS]
```

**Required:**
- `INPUT` - STL file path (or `-` for stdin)
- `-o, --output` - PNG output path (or `-` for stdout)

**View (mutually exclusive):**
- `--view <PRESET>` - front|back|left|right|top|bottom|iso (default: iso)
- `--azimuth <DEG> --elevation <DEG>` - custom angles

**Image:**
- `--width <PX>` - output width (default: 512)
- `--height <PX>` - output height (default: 512)
- `--padding <RATIO>` - padding around model (default: 0.08)
- `--aa <LEVEL>` - anti-aliasing: none|2x|4x (default: 2x)

**Appearance:**
- `--background <TYPE>` - transparent|solid (default: transparent)
- `--background-color <HEX>` - e.g., "#ffffff"
- `--material-color <HEX>` - e.g., "#cccccc"
- `--lighting <PRESET>` - flat|studio|technical (default: studio)

**Output:**
- `--metadata <PATH>` - write render metadata JSON
- `--quiet` - suppress progress output
- `--verbose` - detailed progress

**Exit codes:**
- 0: Success
- 1: Usage error
- 2: Input error
- 3: Output error

---

## Milestone Checklist

Development follows TDD: write failing test first, then implement.

### M0: Project Setup ✓

**Goal:** Repository ready for development.

#### Rust Project
- [x] Initialize cargo project: `cargo init --name stl-render`
- [x] Add dependencies to Cargo.toml
- [x] Create module structure (empty files)
- [x] Setup `cargo test` and `cargo clippy` in CI
- [x] Add .gitignore for Rust + Python artifacts

#### Test Fixtures
- [x] Verify uv is installed: `uv --version`
- [x] Generate fixtures: `cd tools/fixtures && uv run generate_fixtures.py -o ../../fixtures`
- [x] Verify fixtures exist: cube.stl, sphere.stl, etc.
- [x] Create `fixtures/golden/` directory for reference PNGs

---

### M1: Walking Skeleton ✓

**Goal:** End-to-end pipeline works with placeholders.

#### CLI (cli.rs)
- [x] **Test:** parse minimal args `stl-render input.stl -o out.png`
- [x] **Test:** parse all flags with valid values
- [x] **Test:** reject missing required args (clap handles)
- [x] **Test:** reject --view combined with --azimuth
- [x] Implement clap derive struct
- [x] Implement validation logic
- [x] Implement RenderConfig builder

#### STL Detection (stl/parser.rs)
- [x] **Test:** detect binary STL from magic bytes
- [x] **Test:** detect ASCII STL from "solid" prefix
- [x] **Test:** reject empty file
- [x] Implement format detection

#### Placeholder Output (output.rs)
- [x] **Test:** write solid color PNG to file
- [x] **Test:** write PNG to stdout when path is "-"
- [x] Implement write_png with placeholder image

#### Main Integration (main.rs)
- [x] **Test:** exit code 0 on success
- [x] **Test:** exit code 1 on bad args
- [x] **Test:** exit code 2 on missing input file
- [x] Wire up CLI → placeholder PNG output

**Acceptance:** `stl-render cube.stl -o test.png` produces a solid gray PNG. ✓

---

### M2: STL Parsing

**Goal:** Parse binary and ASCII STL into triangle stream.

#### Binary Parser (stl/binary.rs)
- [ ] **Test:** parse single triangle
- [ ] **Test:** parse 1000 triangles, verify count
- [ ] **Test:** handle file with zero triangles
- [ ] **Test:** error on truncated file
- [ ] **Test:** error on triangle count mismatch
- [ ] Implement BinaryStlIter

#### ASCII Parser (stl/ascii.rs)
- [ ] **Test:** parse single triangle
- [ ] **Test:** parse multi-triangle file
- [ ] **Test:** handle various whitespace formats
- [ ] **Test:** handle scientific notation in coords
- [ ] **Test:** error on malformed facet
- [ ] **Test:** error on unclosed solid
- [ ] Implement AsciiStlIter

#### Streaming Interface (stl/mod.rs)
- [ ] **Test:** StlReader::open detects format correctly
- [ ] **Test:** iterate binary STL, collect triangles
- [ ] **Test:** iterate ASCII STL, collect triangles
- [ ] **Test:** triangle_count() returns Some for binary, None for ASCII
- [ ] Implement StlReader with mmap

#### Fixture Verification
Fixtures are generated by `tools/fixtures/generate_fixtures.py` (see M0).
- [ ] **Test:** cube.stl has 12 triangles
- [ ] **Test:** cube_ascii.stl parses identically to cube.stl
- [ ] **Test:** single_triangle.stl has 1 triangle
- [ ] **Test:** empty.stl has 0 triangles (valid)
- [ ] **Test:** truncated.stl returns error

**Acceptance:** Parse cube.stl, verify 12 triangles with correct vertices.

---

### M3: Bounding Box & Normals

**Goal:** Compute geometry metadata from triangle stream.

#### Bounding Box (mesh.rs)
- [ ] **Test:** BoundingBox::new() is empty/invalid
- [ ] **Test:** extend() with single point
- [ ] **Test:** extend() accumulates min/max correctly
- [ ] **Test:** center() returns midpoint
- [ ] **Test:** dimensions() returns size
- [ ] Implement BoundingBox

#### Normal Computation (mesh.rs)
- [ ] **Test:** compute_normal for XY plane triangle
- [ ] **Test:** compute_normal for degenerate triangle (returns zero)
- [ ] **Test:** normal is unit length
- [ ] Implement compute_normal

#### Streaming Bounds
- [ ] **Test:** compute bounds from StlReader without storing mesh
- [ ] Implement bounds_from_reader() helper

**Acceptance:** Compute bounds for cube.stl, verify min/max corners.

---

### M4: Camera Math

**Goal:** View transforms and orthographic projection.

#### View Presets (camera.rs)
- [ ] **Test:** Front view looks down -Z axis
- [ ] **Test:** Iso view is 45° azimuth, 35° elevation
- [ ] **Test:** All 7 presets produce valid matrices
- [ ] Implement ViewPreset enum and view_matrix()

#### Custom Angles (camera.rs)
- [ ] **Test:** azimuth 0, elevation 0 equals Front
- [ ] **Test:** azimuth rotates around Y axis
- [ ] **Test:** elevation tilts up/down
- [ ] Implement from_angles()

#### Auto-Framing (camera.rs)
- [ ] **Test:** unit cube fits in 512x512 with padding
- [ ] **Test:** tall object (1x10x1) is centered vertically
- [ ] **Test:** wide object (10x1x1) is centered horizontally
- [ ] **Test:** padding 0.0 fills entire image
- [ ] **Test:** padding 0.1 leaves 10% margin
- [ ] Implement orthographic projection with auto-fit

**Acceptance:** Render cube from iso view, model is centered with padding.

---

### M5: Software Rasterizer

**Goal:** Render triangles to depth/color buffer.

#### Framebuffer (render.rs)
- [ ] **Test:** new() initializes with background color
- [ ] **Test:** new() with transparent background has alpha=0
- [ ] **Test:** dimensions match requested size
- [ ] Implement Framebuffer struct

#### Triangle Rasterization (render.rs)
- [ ] **Test:** rasterize triangle covering full screen
- [ ] **Test:** triangle outside viewport is clipped
- [ ] **Test:** overlapping triangles use depth test
- [ ] **Test:** backface culling (optional, may skip)
- [ ] Implement rasterize_triangle with barycentric coords

#### Flat Shading (render.rs)
- [ ] **Test:** triangle facing light is bright
- [ ] **Test:** triangle facing away is dark
- [ ] **Test:** material color affects output
- [ ] Implement basic diffuse shading

#### Integration
- [ ] **Test:** render cube.stl, verify non-empty image
- [ ] **Test:** determinism: same input produces identical bytes
- [ ] Wire up full pipeline: STL → bounds → camera → render → PNG

**Acceptance:** `stl-render cube.stl -o cube.png` produces recognizable cube.

---

### M6: Lighting Presets

**Goal:** Multiple lighting configurations.

#### Presets (render.rs)
- [ ] **Test:** flat preset has single front light
- [ ] **Test:** studio preset has key + fill + rim
- [ ] **Test:** technical preset has uniform lighting
- [ ] Implement LightingPreset enum
- [ ] Implement light direction(s) per preset

#### Material Color
- [ ] **Test:** --material-color "#ff0000" produces red output
- [ ] **Test:** hex parsing with/without # prefix
- [ ] Implement color parsing in cli.rs

**Acceptance:** Render with each lighting preset, visible difference.

---

### M7: Anti-Aliasing

**Goal:** Supersampled rendering for smooth edges.

#### SSAA (render.rs)
- [ ] **Test:** 2x renders at double resolution
- [ ] **Test:** 4x renders at quadruple resolution
- [ ] **Test:** none skips downsampling
- [ ] **Test:** downsampled image has correct dimensions
- [ ] Implement render at Nx, box filter downsample

#### Visual Quality
- [ ] **Test (golden):** diagonal edge is smooth with 2x AA
- [ ] Compare none vs 2x on diagonal line

**Acceptance:** Cube edges are noticeably smoother with 2x AA.

---

### M8: Full CLI Surface

**Goal:** All documented flags work correctly.

#### Background Options
- [ ] **Test:** --background transparent has alpha channel
- [ ] **Test:** --background solid uses --background-color
- [ ] **Test:** default background is transparent

#### Stdin/Stdout
- [ ] **Test:** read STL from stdin (pipe)
- [ ] **Test:** write PNG to stdout
- [ ] Handle "-" special path

#### Metadata Output
- [ ] **Test:** --metadata writes JSON file
- [ ] **Test:** JSON contains triangle_count, dimensions, bounds
- [ ] Implement RenderMetadata serialization

#### Verbosity
- [ ] **Test:** --quiet suppresses stdout
- [ ] **Test:** --verbose prints progress for large files
- [ ] Implement output control

**Acceptance:** Every CLI flag documented above works as specified.

---

### M9: Large STL Handling

**Goal:** Process 500MB+ files without excessive memory.

#### Large Fixture Generation
Generate large test files (not committed to repo):
```bash
cd tools/fixtures
uv run generate_fixtures.py -o ../../fixtures --large
# Creates: large_1m.stl (~50MB), large_10m.stl (~500MB)
```

#### Memory Efficiency
- [ ] **Test:** 100MB STL uses <200MB RSS
- [ ] **Test:** 500MB STL completes without OOM
- [ ] Verify two-pass streaming works

#### Performance
- [ ] **Benchmark:** 1M triangles in <2 seconds
- [ ] **Benchmark:** 10M triangles in <20 seconds
- [ ] Profile and optimize hot paths if needed

#### Error Handling
- [ ] **Test:** graceful error on corrupt large file
- [ ] **Test:** progress indication for long renders

**Acceptance:** Render 500MB STL in reasonable time with bounded memory.

---

### M10: Robustness

**Goal:** Handle edge cases gracefully.

#### Invalid Input
- [ ] **Test:** empty file → clear error message
- [ ] **Test:** not an STL → clear error message
- [ ] **Test:** binary STL with wrong triangle count → error
- [ ] **Test:** ASCII STL with syntax error → error with line number

#### Degenerate Geometry
- [ ] **Test:** zero-area triangle is skipped
- [ ] **Test:** NaN/Inf coordinates → error or skip
- [ ] **Test:** model with zero triangles → empty or error

#### File System
- [ ] **Test:** input file doesn't exist → exit code 2
- [ ] **Test:** output directory doesn't exist → exit code 3
- [ ] **Test:** output file not writable → exit code 3

**Acceptance:** Fuzz with malformed inputs, no panics.

---

### M11: Batch Mode

**Goal:** Process multiple files efficiently.

#### Multiple Inputs
- [ ] **Test:** `stl-render *.stl -o outdir/`
- [ ] **Test:** output naming: `input.stl` → `outdir/input.png`
- [ ] Implement glob expansion or multiple positional args

#### Multiple Views
- [ ] **Test:** `--views front,back,iso` produces 3 PNGs
- [ ] **Test:** output naming: `input.front.png`, `input.back.png`
- [ ] Implement multi-view mode

#### Parallelism (optional)
- [ ] Consider rayon for parallel file processing
- [ ] Keep single-file rendering single-threaded for determinism

**Acceptance:** Render 10 STLs to directory in one command.

---

### M12: Library API

**Goal:** Clean public API for embedding.

#### Public Interface (lib.rs)
- [ ] **Test:** call render() from external crate
- [ ] **Test:** RenderConfig is constructible without cli.rs
- [ ] Export: RenderConfig, RenderMetadata, RenderError
- [ ] Export: StlReader, Triangle, BoundingBox
- [ ] Document public API with rustdoc

#### Stability
- [ ] Review API surface for breaking change risk
- [ ] Mark internal modules as pub(crate)

**Acceptance:** Can use as library: `stl_render::render(&config)?`

---

## Golden Image Tests

Store reference PNGs in `fixtures/golden/`. Compare with tolerance for CI.

| Fixture | Views | Notes |
|---------|-------|-------|
| cube.stl | all 7 | Basic geometry, 12 triangles |
| sphere.stl | iso | Curved surface shading, 1280 triangles |
| cylinder.stl | front, top, iso | Mixed flat/curved surfaces |
| tall_column.stl | front, iso | Aspect ratio handling (tall) |
| flat_tile.stl | top, iso | Near-planar model (wide) |
| long_beam.stl | front, top | Elongated geometry |
| single_triangle.stl | front | Minimal geometry |

Golden images generated once rendering is stable. Workflow:
1. Render all fixtures: `stl-render fixtures/*.stl -o fixtures/golden/ --views front,iso`
2. Manually verify images look correct
3. Commit as reference
4. CI compares new renders against reference (pixel diff with tolerance)

---

## Dependencies

```toml
[dependencies]
clap = { version = "4", features = ["derive"] }
glam = "0.27"
image = { version = "0.25", default-features = false, features = ["png"] }
memmap2 = "0.9"
serde = { version = "1", features = ["derive"] }
serde_json = "1"
thiserror = "1"
tiny-skia = "0.11"

[dev-dependencies]
tempfile = "3"
```
