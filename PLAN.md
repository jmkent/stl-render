...
These issues were found during a project state review. Treat them as near-term stabilization work before adding new feature surface.

### K1: Invalid CLI Values Fall Back Silently

**Problem:** Unknown values for `--view`, `--aa`, `--background`, `--lighting`, and color flags currently fall back to defaults instead of failing with a usage error. This contradicts the documented exit code `1` for bad arguments and can hide typos in scripts.

- [ ] **Test:** `--view definitely-not-a-view` exits with code `1`
- [ ] **Test:** `--views front,nope,iso` exits with code `1`
- [ ] **Test:** `--aa nope` exits with code `1`
- [ ] **Test:** `--background gradient` exits with code `1`
- [ ] **Test:** `--lighting dramatic` exits with code `1`
- [ ] **Test:** malformed hex color such as `--material-color xyz` exits with code `1`
- [ ] Change CLI parsing helpers to return `Result<_, CliError>` instead of defaulting
- [ ] Add clear error variants for invalid view, AA, background, lighting, and color values
- [ ] Consider using clap `ValueEnum` for closed option sets where it fits local patterns
...
### K3: `print-grid` Does Not Support Stdin

**Problem:** Normal rendering supports `-` input via `StlReader::from_reader()`, but `render_print_grid()` always calls `StlReader::open()`, so `stl-render - -o out.png --view print-grid < model.stl` fails.

- [ ] **Test:** `--view print-grid` can read binary STL data from stdin
- [ ] **Test:** `--view print-grid` can read ASCII STL data from stdin
- [ ] Refactor reader creation into a shared helper used by single-view and grid rendering
- [ ] Ensure stdin remains disallowed only for true batch mode with multiple inputs or views

**Acceptance:** `print-grid` supports stdin anywhere single-view rendering supports stdin.

### K4: Formatting and Strict Lint Health

**Problem:** The CI workflow includes `cargo fmt --all -- --check`, but the current tree is not formatted. Also, `cargo clippy --all-targets --all-features -- -D warnings` catches test-only lints even though the current CI clippy command is narrower.

- [ ] Run `cargo fmt --all`
- [ ] **Test:** `cargo fmt --all -- --check` passes
- [ ] Fix test lints found by `cargo clippy --all-targets --all-features -- -D warnings`
- [ ] **Test:** `cargo clippy --all-targets --all-features -- -D warnings` passes
- [ ] Update CI clippy command to include `--all-targets`
- [ ] Keep `cargo test` passing after formatting and lint fixes
